...
.summary-header {background:#a5c639 !important;}
...
def _make_summary_table(path: Optional[Path]) -> List[str]:
    """Extract a <table class='summary'> from *path* and return a list
    containing a single HTML string. Returns an empty list if *path* is None
    or no summary table is found.
    """
    if not path:
        return []
    try:
        dfs = pd.read_html(path, attrs={"class": "summary"})
    except Exception:
        return []
    if not dfs:
        return []
    ...
    rows = []
    # Header row
    header_cells = [f'<th class="summary-header">{col}</th>' for col in df.columns]
...
def generate_report(
    ...
    right_title: str,
    output_path: Path,
    left_summary_path: Optional[Path] = None,
    right_summary_path: Optional[Path] = None,
) -> None:
    ...
    * ``left_title`` / ``right_title`` – titles shown above each column.
    ...
    parts = [
        ...
        *(_make_summary_table(left_summary_path) if left_summary_path else []),
        *[_make_table(df) for df in left_dfs],
        "</div>",
        f"<div class='col'><h2>{right_title}</h2>",
        *(_make_summary_table(right_summary_path) if right_summary_path else []),
